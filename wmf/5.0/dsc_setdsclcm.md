...
ms.locfileid: "62057962"
---
# <a name="set-dsclocalconfigurationmanager-cmdlet-supports--force-parameter"></a><span data-ttu-id="7036e-102">El cmdlet Set-DscLocalConfigurationManager admite el parámetro -force</span><span class="sxs-lookup"><span data-stu-id="7036e-102">Set-DscLocalConfigurationManager cmdlet supports -force parameter</span></span>

<span data-ttu-id="7036e-103">Hemos agregado compatibilidad para el nuevo parámetro con el cmdlet Set-DscLocalConfigurationManager.</span><span class="sxs-lookup"><span data-stu-id="7036e-103">We have added a support for new parameter to Set-DscLocalConfigurationManager cmdlet.</span></span> <span data-ttu-id="7036e-104">Permitirá al usuario restablecer la metaconfiguración en la máquina de manera determinista cuando se están ejecutando otras operaciones, como la comprobación de coherencia, en segundo plano, ya que causará que todas las operaciones en ejecución se detengan.</span><span class="sxs-lookup"><span data-stu-id="7036e-104">This will allow the user to reset meta configuration on machine deterministically when other operations like consistency check are running in background as it will cause all running operations to be stopped.</span></span>

<span data-ttu-id="7036e-105">La experiencia tiene el aspecto siguiente al intentar establecer la metaconfiguración sin el parámetro –Force.</span><span class="sxs-lookup"><span data-stu-id="7036e-105">The experience looks like this when trying to set meta configuration without –Force parameter.</span></span>
```powershell
...
<span data-ttu-id="7036e-106">Si usamos –force correctamente, se actualiza la metaconfiguración en el sistema. Para ello, se cancela la operación en ejecución actual en la máquina.</span><span class="sxs-lookup"><span data-stu-id="7036e-106">When we use –force it successfully updates the meta configuration on system by canceling the current running operation on the machine.</span></span>
```powershell
