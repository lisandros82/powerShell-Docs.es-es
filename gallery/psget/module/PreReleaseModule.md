...
* La cadena de versión preliminar se especifica en el manifiesto del módulo, en la sección PSData de PrivateData. A continuación se indican los requisitos detallados de la cadena de versión preliminar. 
...
Para trabajar con elementos de versión preliminar mediante los comandos de PowerShellGet Find-Module, Install-Module, Update-Module y Save-Module es necesario agregar la marca -AllowPrerelease. Si se especifica -AllowPrerelease, se incluyen los elementos de versión preliminar, siempre que estén presentes.
...
Uninstall-Module quita la versión más reciente de un módulo si no se proporciona -RequiredVersion. Si se especifica -RequiredVersion y es una versión preliminar, debe agregarse -AllowPrerelease al comando. 
