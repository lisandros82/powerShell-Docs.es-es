...
El cmdlet `Get-Command` de Windows PowerShell obtiene todos los comandos disponibles en la sesión actual. Cuando se escribe `Get-Command` en un símbolo del sistema de Windows PowerShell, se obtiene un resultado como este:
...
Para obtener los comandos nativos de otros tipos, use el parámetro **CommandType** del cmdlet `Get-Command`.
