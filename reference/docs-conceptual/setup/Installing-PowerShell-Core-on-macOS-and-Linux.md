...
Descargue el paquete de Debian `powershell_6.0.0-rc-1.ubuntu.14.04_amd64.deb` desde la página de [versiones][] en la máquina Ubuntu.
...
Descargue el paquete de Debian `powershell_6.0.0-rc-1.ubuntu.16.04_amd64.deb` desde la página de [versiones][] en la máquina Ubuntu.
...
Descargue el paquete de Debian `powershell_6.0.0-rc-1.ubuntu.17.04_amd64.deb` desde la página de [versiones][] en la máquina Ubuntu.
...
Descargue el paquete de Debian `powershell_6.0.0-rc-1.debian.8_amd64.deb` desde la página de [versiones][] en la máquina Debian.
...
Descargue el paquete de Debian `powershell_6.0.0-rc-1.debian.9_amd64.deb` desde la página de [versiones][] en la máquina Debian.
...
Cuando use [CentOS 7][], descargue el paquete RPM `powershell-6.0.0_rc-1.rhel.7.x86_64.rpm` desde la página de [versiones][] en la máquina CentOS.
...
Descargue el paquete RPM `powershell-6.0.0_rc-1.rhel.7.x86_64.rpm` desde la página de [versiones][] en la máquina Red Hat Enterprise Linux.
...
> **Nota**: Al instalar PowerShell Core, OpenSUSE puede informar de que no hay nada que proporcione `libcurl`.
`libcurl` ya debería estar instalado en las versiones compatibles de OpenSUSE.
Ejecute `zypper search libcurl` para confirmarlo.
El error ofrecerá dos soluciones. Elija la solución 2 para continuar con la instalación de PowerShell Core.
...
Descargue el paquete RPM `powershell-6.0.0_rc-1.rhel.7.x86_64.rpm` desde la página de [versiones][] en la máquina OpenSUSE.
...
Descargue el paquete RPM `powershell-6.0.0_rc-1.rhel.7.x86_64.rpm` desde la página de [versiones][] en la máquina Fedora.
...
Si usa una distribución de Linux reciente, descargue el archivo AppImage `powershell-6.0.0-rc-x86_64.AppImage` desde la página de [versiones][] en el equipo Linux.
...
Este paquete funciona con independencia de la distribución de Linux del usuario y es un solo archivo binario.
...
[Homebrew][brew] es el administrador de paquetes que faltan de macOS.
...
Cuando use macOS 10.12, descargue el paquete PKG `powershell-6.0.0-rc-osx.10.12-x64.pkg` desde la página de [versiones][] en la máquina macOS.
...
Para desinstalar las rutas de acceso de PowerShell adicionales (por ejemplo, la ruta de acceso al perfil de usuario), vea más adelante en este documento la sección [Rutas de acceso][paths] y elimine las rutas de acceso deseadas con `sudo rm`.
(Nota: Esto no es necesario si ha realizado la instalación con Homebrew).
...
Para implementar los archivos binarios de PowerShell en las distribuciones de Linux que no se admiten oficialmente, deberá instalar las dependencias necesarias para el sistema operativo de destino en pasos independientes.
...
* `$PSHOME` es `/opt/microsoft/powershell/6.0.0-rc/`.
...
Tenga en cuenta que, dado que macOS es una derivación de BSD, el prefijo que se usa es `/usr/local` en lugar de `/opt`.
Por lo tanto, `$PSHOME` es `/usr/local/microsoft/powershell/6.0.0-rc/`, y el vínculo simbólico se coloca en `/usr/local/bin/pwsh`.
