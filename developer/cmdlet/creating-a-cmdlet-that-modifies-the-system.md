...
Debe llamar la entrada que procesar el método de su cmdlet la [System.Management.Automation.Cmdlet.Shouldprocess*](/dotnet/api/System.Management.Automation.Cmdlet.ShouldProcess) para confirmar la ejecución de una operación antes de que se realiza un cambio (por ejemplo, la eliminación de archivos) en el que se ejecuta (método) estado del sistema. Esto permite que el tiempo de ejecución de Windows PowerShell proporcionar el comportamiento correcto de "WhatIf" y "Confirmar" en el shell.
