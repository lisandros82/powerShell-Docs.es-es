...
- Los módulos dinámicos creados por el `New-Module` cmdlet se pueden denominar mediante `Name` el parámetro del cmdlet.

- Los módulos importados desde objetos de ensamblado mediante el `"dynamic_code_module_" + assembly.GetName()`  **`Import-Module` comando-Assembly** se denominan según la sintaxis siguiente:.
...
Cmdlet [Export-ModuleMember:](/powershell/module/Microsoft.PowerShell.Core/Export-ModuleMember) este cmdlet especifica los miembros del módulo (por ejemplo, cmdlets, funciones, variables y alias) que se exportan desde un archivo de módulo de script (. psm1) o desde un módulo `New-Module` dinámico creado mediante el cmdlet.
